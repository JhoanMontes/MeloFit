            ...
            <>
                ...
                {loadingMembers ? (<ActivityIndicator color="#2563EB" size="small" />) : members.length === 0 ? (
                    ...
                    <View className="space-y-3">
                        ...
                        return (
                            <Pressable key={member.id} onPress={() => navigation.navigate('AthleteDetail', { athlete: member })} onLongPress={() => toggleMember(member.id)} className={`p-4 rounded-2xl flex-row items-center justify-between shadow-sm border ${isSelected ? 'bg-red-50 border-red-200' : 'bg-white border-gray-100'}`}>
                                <View className="flex-row items-center flex-1 mr-4">
                                    <View className={`w-10 h-10 rounded-full items-center justify-center mr-3 ${isSelected ? 'bg-red-100' : 'bg-gray-50'}`}><Ionicons name={isSelected ? "trash-outline" : "person-outline"} size={20} color={isSelected ? '#DC2626' : '#6B7280'} /></View>
                                    <View className="flex-1"><Text className={`font-bold text-base ${isSelected ? 'text-red-900' : 'text-gray-900'}`}>{member.name}</Text><View className="flex-row items-center mt-0.5"><Ionicons name="card-outline" size={12} color="#9CA3AF" /><Text className="text-gray-400 text-xs ml-1">Doc: {member.id}</Text></View></View>
                                </View>
                                {isSelected ? <Ionicons name="close-circle" size={24} color="#DC2626" /> : <Ionicons name="chevron-forward" size={20} color="#E5E7EB" />}
                            </Pressable>
                        );
                        })}
                    </View>
            ...
            <>
                ...
                <View className="space-y-3 mb-6">
                    {mockGroupTests.active.map((test) => (
                        <Pressable 
                            key={test.id} 
                            // Navegar al detalle para ver quien falta (tab: 'pending')
                            onPress={() => navigation.navigate('TestAssignmentDetail', { assignmentId: test.id, testName: test.name, groupName: groupName, initialTab: 'pending' })}
                            className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 active:bg-gray-50"
                        >
                            <View className="flex-row justify-between items-center mb-2">
                                <View className="flex-row items-center gap-3">
                                    <View className="w-10 h-10 bg-orange-50 rounded-full items-center justify-center"><Ionicons name="hourglass" size={20} color="#F97316" /></View>
                                    <View><Text className="font-bold text-slate-900">{test.name}</Text><Text className="text-xs text-slate-500">Vence: {test.deadline}</Text></View>
                                </View>
                                <Ionicons name="chevron-forward" size={20} color="#E5E7EB" />
                            </View>
                            <View>
                                <View className="flex-row justify-between mb-1"><Text className="text-[10px] text-slate-400 font-bold uppercase">Progreso</Text><Text className="text-[10px] text-blue-600 font-bold">{test.completedCount}/{members.length > 0 ? members.length : 1} completados</Text></View>
                                <View className="h-1.5 bg-slate-100 rounded-full overflow-hidden"><View className="h-full bg-blue-500 w-[40%]" /></View>
                            </View>
                        </Pressable>
                    ))}
                </View>
                ...
                <View className="space-y-3">
                    {mockGroupTests.history.map((test) => (
                        <Pressable 
                            key={test.id}
                            // Navegar al detalle para ver resultados (tab: 'completed')
                            onPress={() => navigation.navigate('TestAssignmentDetail', { assignmentId: test.id, testName: test.name, groupName: groupName, initialTab: 'completed' })}
                            className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 opacity-80 flex-row justify-between items-center active:bg-gray-50"
                        >
                            <View className="flex-row items-center gap-3">
                                <View className="w-10 h-10 bg-emerald-50 rounded-full items-center justify-center"><Ionicons name="checkmark-done" size={20} color="#10B981" /></View>
                                <View><Text className="font-bold text-slate-900">{test.name}</Text><Text className="text-xs text-slate-500">{test.date}</Text></View>
                            </View>
                            <View className="flex-row items-center">
                                <View className="items-end mr-2"><Text className="text-emerald-700 font-bold text-sm">Completado</Text><Text className="text-xs text-slate-400">{test.completedCount} entregas</Text></View>
                                <Ionicons name="chevron-forward" size={20} color="#E5E7EB" />
                            </View>
                        </Pressable>
                    ))}
      ...
      <Modal visible={showAddMemberModal} transparent animationType="slide" onRequestClose={() => setShowAddMemberModal(false)}>
        <View className="flex-1 bg-white mt-20 rounded-t-[32px] shadow-2xl border-t border-slate-100">
            <View className="p-6">
                ...
                <View className="flex-row gap-3 mb-6">
                    <TextInput 
                        value={searchDoc} onChangeText={setSearchDoc} placeholder="Ej. 100123456" keyboardType="numeric" placeholderTextColor="#9CA3AF"
                        style={[styles.safeInput, { flex: 1, marginBottom: 0 }]}
                    />
                    <Pressable onPress={handleSearchAthlete} className="bg-blue-600 w-14 rounded-xl items-center justify-center active:bg-blue-700">{searching ? <ActivityIndicator color="white" /> : <Ionicons name="search" size={24} color="white" />}</Pressable>
                </View>
                ...
                {foundAthlete && (
                    <View className="bg-blue-50 border border-blue-100 p-5 rounded-2xl">
                        <View className="flex-row items-center mb-4">
                            <View className="w-12 h-12 bg-blue-100 rounded-full items-center justify-center mr-4"><Ionicons name="person" size={24} color="#2563EB" /></View>
                            <View><Text className="text-lg font-bold text-slate-900">{foundAthlete.nombre_completo}</Text><Text className="text-slate-500 font-medium">Doc: {foundAthlete.no_documento}</Text></View>
                        </View>
                        <Pressable onPress={handleAddFoundAthlete} className="bg-blue-600 h-12 rounded-xl flex-row items-center justify-center active:bg-blue-700"><Ionicons name="add-circle-outline" size={20} color="white" style={{marginRight: 8}} /><Text className="text-white font-bold text-base">Agregar al Grupo</Text></Pressable>
                    </View>
                )}
            </View>
        </View>

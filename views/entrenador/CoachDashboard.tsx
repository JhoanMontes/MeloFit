    ...
    useCallback(() => {
      ...
      const loadAssignments = async () => {
        try {
          ...
          // 4. Tomar solo la última asignación de cada prueba
          const map = {};
          asignaciones.forEach(a => {
            const pruebaId = a.prueba_asignada.prueba.id;
            const fecha = new Date(a.prueba_asignada.fecha_asignacion);

            if (!map[pruebaId] || fecha > new Date(map[pruebaId].prueba_asignada.fecha_asignacion)) {
              map[pruebaId] = a;
            }
          });
          ...
          // 6. Formato final con datos REALES desde Supabase
          const finalData = await Promise.all(
            latest.map(async (a) => {
              ...
              const { count: totalCount } = await supabase
                ...
                .eq("prueba_asignada_id", a.prueba_asignada.id);
              ...
              return {
                id: a.prueba_asignada.prueba.id,

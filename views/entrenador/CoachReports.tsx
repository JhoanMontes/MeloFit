  ...
  const createHTMLReport = (
    ...
    return `
      ...
      <body>
          ...
          <div class="section">
            ...
            <table>
              ...
              <tbody>
                ${physicalData.map(d => `
                  <tr>
                <td>${d.fecha_registro ? d.fecha_registro.split('T')[0].split('-').reverse().join('/') : '-'}</td>
                    <td>${d.peso || '-'}</td>
                    <td>${d.estatura || '-'}</td>
                    <td>${d.imc || '-'}</td>
            ...
            <table>
              ...
              <tbody>
                ${performanceData.map(r => `
                  <tr>
                   <td>${r.fecha_realizacion ? r.fecha_realizacion.split('T')[0].split('-').reverse().join('/') : '-'}</td>
        ...
        <div class="footer">
          Reporte generado automáticamente por SportSync App.
        </div>
      </body>
      </html>
    `;
    ...
    try {
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, {
          ...
          dialogTitle: 'Compartir Reporte SportSync'
        });
      } else {
